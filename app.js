...
async function loadData() {
    try {
        ...
        // --- BLOQUEO POR DEFECTO: LONG JUMP ---
        if (disciplineSelect) {
            disciplineSelect.value = "Long Jump";
            disciplineSelect.style.display = "none"; // Ocultar el selector original
            
            // Añadir la etiqueta de "SOLO LONG JUMP"
            const label = document.createElement('div');
            label.innerHTML = '<span style="background:rgba(0,112,243,0.15); border:1px solid #0070f3; color:#0070f3; padding:8px 12px; border-radius:8px; font-size:0.8rem; font-weight:bold;"><i class="fas fa-lock"></i> SOLO LONG JUMP</span>';
            disciplineSelect.parentNode.appendChild(label);
        }
        // --------------------------------------
...
// 7. MODAL CON AVISO DE "POR CONFIRMAR" (TU CÓDIGO ORIGINAL)
...
// (Discipline Select ya no tiene listener porque está oculto)
...
if (loginForm) {
    loginForm.addEventListener('submit', async (e) => {
        e.preventDefault(); 
        const email = document.getElementById('login-email').value; // CORREGIDO el ID para que coincida con tu HTML
        const password = document.getElementById('login-password').value; // CORREGIDO el ID
